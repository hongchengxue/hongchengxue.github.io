...
- 写作台是全屏写作模式：**Typora 式所见即所得**（Vditor 即时渲染，点击段落就地显示 Markdown 源码）、编辑区铺满屏幕、Tab 键插入 4 空格、支持**一键全屏**
- **图片粘贴即上传**：从 Word / Typora / 截图工具复制图片，在编辑器里直接 Ctrl+V 即可，图片自动上传到仓库并插入正文；拖拽图片进编辑器同样自动上传
- 写作台编辑器是 **Typora 式所见即所得**（Vditor 即时渲染）：输入即渲染成文，**点击某个段落，该段就地显示 Markdown 源码**，点击其他位置恢复渲染；工具栏提供标题/加粗/列表/表格/图片上传/视频/全屏等能力，中文输入法友好
